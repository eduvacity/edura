...
// Zod schema for validating a local PDF file (only PDF)
...
export default function PdfUpload({ moduleId }: { moduleId: string }) {
  ...
  // Dummy cloud files for demonstration (using PDF samples)
      ...
      if (provider === "dropbox") {
        ...
        window.Dropbox.choose({
          success: (files: any) => {
            ...
            const file = files[0]
            // Update state assuming file.link is a URL to the PDF file
                      ...
                      const picker = new window.google.picker.PickerBuilder()
                        ...
                        .setCallback((data: any) => {
                          if (
                            ...
                            // Assume doc.url is a link to the PDF file
                            setSelectedCloudFile({
                              id: doc.id,
                              name: doc.name,
                              previewUrl: doc.url,
  ...
  const handleSubmit = async () => {
    if (selectedFile) {
      ...
      try {
        ...
        alert("Local PDF uploaded successfully!")
        setSelectedFile(null)
        setPreviewUrl(null)
        setIsSubmitted(true)
      } catch (error: any) {
      ...
      try {
        const response = await fetch("/api/upload-pdf-from-cloud", {
          method: "POST",
        ...
        if (!response.ok) throw new Error("Failed to submit cloud PDF.")
        alert("Cloud PDF submitted successfully!")
        setSelectedCloudFile(null)
        setPreviewUrl(null)
      } catch (error: any) {
        alert("Upload error: " + error.message)
          ...
          <div className="w-full flex flex-col gap-[10px]">
            <div className="w-full max-w-[636px] flex flex-col gap-[10px]">
              <p className="font-arial font-normal text-base leading-[27.2px] tracking-normal text-[#535353]">
                Upload a PDF that can be viewed in your lesson. You can also
