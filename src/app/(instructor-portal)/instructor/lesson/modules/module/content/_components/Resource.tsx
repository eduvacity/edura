...
// Zod schema for validating a local resource file (video or audio)
const resourceFileSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Invalid file type. Please upload a video or audio file (mp4, mov, avi, mp3).",
    }
  )
...
export default function ResourceUpload({ moduleId }: { moduleId: string }) {
  ...
  // New state for the pasted link (if needed)
  ...
  const dummyCloudFiles: CloudFile[] = [
    {
      id: "1",
      name: "CloudResource1.mp4",
      previewUrl:
        "https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4",
    },
    {
      id: "2",
      name: "CloudResource2.mp3",
      previewUrl: "https://www.sample-videos.com/audio/mp3/crowd-cheering.mp3",
    },
  ]
  ...
  const handleFiles = (files: FileList) => {
    const file = files[0]
    const result = resourceFileSchema.safeParse(file)
    if (!result.success) {
      alert(result.error.errors[0].message)
      return
    }
    console.log("Valid resource file selected:", file)
  ...
  const handleSubmit = async () => {
    if (selectedFile) {
      const result = resourceFileSchema.safeParse(selectedFile)
      if (!result.success) {
        alert(result.error.errors[0].message)
        return
      }
      ...
      try {
        ...
        alert("Local resource uploaded successfully!")
        setSelectedFile(null)
        setPreviewUrl(null)
        setIsSubmitted(true)
      } catch (error: any) {
      ...
      try {
        const response = await fetch("/api/upload-resource-from-cloud", {
          method: "POST",
        ...
        if (!response.ok) throw new Error("Failed to submit cloud resource.")
        alert("Cloud resource submitted successfully!")
        setSelectedCloudFile(null)
        setPreviewUrl(null)
      } catch (error: any) {
        alert("Upload error: " + error.message)
          ...
          <div className="w-full flex flex-col gap-[10px]">
            <div className="w-full max-w-[636px] flex flex-col gap-[10px]">
              <p className="font-arial font-normal text-base leading-[27.2px] tracking-normal text-[#535353]">
                Upload resources that can be played in your lesson. You can also
                include the link for your student to download the file. Max file
                size is 2GB.
              </p>
            </div>
            {/* Hidden file input */}
            <input
              ...
              ref={fileInputRef}
              className="hidden"
              accept="video/mp4,video/quicktime,video/x-msvideo,audio/mpeg"
            ...
            <div className="w-full max-w-[1499px] min-h-[431px] border border-solid border-[#DDDDDD] rounded-2xl flex rounded-bl-[0] rounded-tl-[0]">
              ...
              <div className="min-h-[431px] h-full w-[80px] border-r border-solid border-[#DDDDDD] bg-[#F8F8F8] grid place-content-center relative z-20 pointer-events-auto">
                <div className="w-[64px] h-[272px] flex flex-col gap-10">
                  <div
                    className="w-[64px] h-[64px] rounded-full bg-[#E9E9E9] flex justify-center items-center cursor-pointer hover:bg-[#E0FAEC] transition-colors"
                ...
                {isDragging ? (
                  <div className="w-[254px] h-[93px] flex flex-col gap-[9px] justify-center items-center">
                    ...
                    <p className="font-arial font-normal tracking-normal text-xl leading-[23px] text-[#4D4D4D]">
                      Drop your resource file to upload
                    </p>
                  </div>
                ) : (
                  <div className="w-[254px] h-[93px] flex flex-col gap-[9px] justify-center items-center cursor-pointer">
                    ...
                    <span className="font-arial font-normal text-[10px] leading-[18px] tracking-normal text-center text-[#475467]">
                      Supported file: mp4, mov, avi, mp3
                    </span>
                  </div>
                )}
              </div>
