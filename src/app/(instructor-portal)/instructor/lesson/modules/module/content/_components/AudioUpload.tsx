...
// Zod schema for validating a local audio file (only mp3)
...
export default function AudioUpload({ moduleId }: { moduleId: string }) {
      ...
      if (provider === "dropbox") {
        ...
        window.Dropbox.choose({
          success: (files: any) => {
            ...
            const file = files[0]
            // Update state assuming file.link is a URL to the audio file
                      ...
                      const picker = new window.google.picker.PickerBuilder()
                        ...
                        .setCallback((data: any) => {
                          if (
                            ...
                            // Assume doc.url is a link to the audio file
                            setSelectedCloudFile({
                              id: doc.id,
                              name: doc.name,
                              previewUrl: doc.url,
  ...
  const handleSubmit = async () => {
    if (selectedFile) {
      const result = audioFileSchema.safeParse(selectedFile)
      if (!result.success) {
        alert(result.error.errors[0].message)
        return
      }
      ...
      try {
        ...
        alert("Local audio uploaded successfully!")
        setSelectedFile(null)
        setPreviewUrl(null)
        setIsSubmitted(true)
      } catch (error: any) {
      ...
      try {
        const response = await fetch("/api/upload-audio-from-cloud", {
          method: "POST",
        ...
        if (!response.ok) throw new Error("Failed to submit cloud audio.")
        alert("Cloud audio submitted successfully!")
        setSelectedCloudFile(null)
        setPreviewUrl(null)
      } catch (error: any) {
        alert("Upload error: " + error.message)
          ...
          <div className="w-full flex flex-col gap-[10px]">
            <div className="w-full max-w-[636px] flex flex-col gap-[10px]">
              <p className="font-arial font-normal text-base leading-[27.2px] tracking-normal text-[#535353]">
                Upload audio that can be played in your lesson. You can also
