...
// Zod schema for validating a local video file
const videoFileSchema = z
  ...
  .refine(
    ...
    {
      message: "Invalid file type. Please upload a video file (mp4, mov, avi).",
    }
  )
...
export default function VideoUpload({ moduleId }: { moduleId: string }) {
  ...
  // New state for the pasted link in embedded mode
  const [pastedLink, setPastedLink] = useState<string>("")
  const fileInputRef = useRef<HTMLInputElement>(null)

  // Dummy cloud files for demonstration
  const dummyCloudFiles: CloudFile[] = [
    {
      id: "1",
      name: "CloudVideo1.mp4",
      previewUrl:
        "https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4",
    },
    {
      id: "2",
      name: "CloudVideo2.mp4",
      previewUrl:
        "https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4",
    },
  ]
  ...
  const handleFiles = (files: FileList) => {
    ...
    console.log("Valid local file selected:", file)
      ...
      if (provider === "dropbox") {
        ...
        window.Dropbox.choose({
          success: (files: any) => {
            ...
            const file = files[0]
            // Update state assuming file.link is a URL to the video
                      ...
                      const picker = new window.google.picker.PickerBuilder()
                        ...
                        .setCallback((data: any) => {
                          if (
                            ...
                            // Assume doc.url is a link to the video file
                            setSelectedCloudFile({
                              id: doc.id,
                              name: doc.name,
                              previewUrl: doc.url,
  ...
  const handleSubmit = async () => {
    if (active === 1) {
      // Local file submission using FormData or cloud file submission
      if (selectedFile) {
        const result = videoFileSchema.safeParse(selectedFile)
        if (!result.success) {
          alert(result.error.errors[0].message)
          return
        }
        ...
        try {
          ...
          alert("Local video uploaded successfully!")
          setSelectedFile(null)
          setPreviewUrl(null)
          setIsSubmitted(true)
        } catch (error: any) {
        ...
        try {
          const response = await fetch("/api/upload-video-from-cloud", {
            method: "POST",
          ...
          if (!response.ok) throw new Error("Failed to submit cloud video.")
          alert("Cloud video submitted successfully!")
          setSelectedCloudFile(null)
          setPreviewUrl(null)
        } catch (error: any) {
          alert("Upload error: " + error.message)
      ...
      try {
        // const response = await fetch("/api/upload-embedded-video", {
        //   method: "POST",
        //   headers: { "Content-Type": "application/json" },
        //   body: JSON.stringify({ videoLink: pastedLink }),
        // })
        if (pastedLink) {
          // if (!response.ok) throw new Error("Failed to submit embedded video.")
          setIsSubmitted(true)
          setPastedLink("")
        }
      } catch (error: any) {
          ...
          <div className="w-full flex flex-col gap-[10px]">
            <div className="w-full max-w-[636px] flex flex-col gap-[10px]">
              <div className="w-full flex gap-[7px]">
                <button
                  className={`w-fit h-[42px] flex justify-center items-center gap-[7.69px] border-[0.77px] border-solid ${
                    active === 1
                      ? "border-[#3FA46E] bg-[#E0FAEC]"
                      : "border-[#E3E3E3] rounded-[17px] bg-transparent"
                  } rounded-[17px] py-3 px-[15px] font-arial font-normal text-base leading-[18.4px] tracking-normal text-[#545454] uppercase`}
                  onClick={() => {
                    setActive(1)
                    // Clear pasted link when switching modes
                    setPastedLink("")
                  }}
                >
                  Upload
                ...
                <button
                  ...
                  onClick={() => {
                    setActive(2)
                    // Clear any file selections when switching to embed mode
                    setSelectedFile(null)
                    setSelectedCloudFile(null)
                    setPreviewUrl(null)
                  }}
              ...
              {active === 1 ? (
                <p className="font-arial font-normal text-base leading-[27.2px] tracking-normal text-[#535353]">
                  Upload video that can be played in your lesson. You can also
                  include the link for your student to download the file. Max
                  file size is 2GB.
                </p>
              ) : active === 2 ? (
                <p className="font-arial font-normal text-base leading-[27.2px] tracking-normal text-[#535353]">
                  Embed video from YouTube or Vimeo into your curriculum.
                </p>
              ) : null}
            </div>
            {active === 1 ? (
                    ...
                    {isDragging ? (
                      ...
                      <div className="w-[254px] h-[93px] flex flex-col gap-[9px] justify-center items-center cursor-pointer">
                        ...
                        <span className="font-arial font-normal text-[10px] leading-[18px] tracking-normal text-center text-[#475467]">
                          Supported file include mp4, mov and avi
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  {dummyCloudFiles.map((file) => (
                    <li
                      ...
                      onClick={() => {
                        ...
                        // Clear pasted link when a cloud file is chosen
                        setPastedLink("")
